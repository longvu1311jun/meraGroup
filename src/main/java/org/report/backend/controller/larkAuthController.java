...
//  }
//
//  // Callback sau khi user ấn Authorize
...
//
//    // Nếu không có code, có nghĩa là lỗi hoặc chưa login
//    if (code == null || code.isEmpty()) {
//      redirectAttributes.addFlashAttribute("error", "Không nhận được authorization code. Vui lòng đăng nhập lại.");
...
//
//      // Lưu token vào TokenStorageService để scheduler có thể tự động làm mới
...
//      );
//      log.info("✅ Token đã được lưu vào TokenStorageService - Scheduler sẽ tự động làm mới mỗi 1 giờ");
//
//      // Bước 2: Tự động lấy app token từ wiki
//      try {
...
//
//        // Bước 2.5: Lấy danh sách Base IDs từ Wiki
//        try {
...
//          log.info("✅ Đã lấy được {} Base IDs từ Wiki", baseIds.size());
//          // In ra console danh sách Base IDs
//          log.info("═══════════════════════════════════════════════════════════════════════════");
//          log.info("📋 DANH SÁCH BASE IDs:");
//          log.info("═══════════════════════════════════════════════════════════════════════════");
//          for (larkAuthService.BaseIdInfo baseId : baseIds) {
//            log.info("│ Base Name: {}", baseId.getBaseName());
//            log.info("│ Base ID:   {}", baseId.getBaseId());
//            log.info("│ Path:      {}", baseId.getPath());
//            log.info("├───────────────────────────────────────────────────────────────────────────");
//          }
//          log.info("═══════════════════════════════════════════════════════════════════════════");
//        } catch (Exception e) {
//          log.warn("⚠️ Không thể lấy danh sách Base IDs: {}", e.getMessage());
//          session.setAttribute("baseIdsError", e.getMessage());
//        }
//
//        // Lưu token và app token vào properties để các service khác sử dụng (phải set TRƯỚC khi gọi getTableIDService)
//        if (reportProps != null) {
//          reportProps.setUserAccessToken(userAccessToken);
//          reportProps.setAppToken(appToken);
//        }
//
//        // Bước 3: Lấy danh sách tables (filter tên chứa "_")
...
//
//        // Bước 3.5: Lấy danh sách Base IDs và Table IDs từ getTableID service
...
//            log.info("");
//            log.info("═══════════════════════════════════════════════════════════════════════════");
//            log.info("📋 DANH SÁCH BASE IDs VÀ TABLE IDs:");
//            log.info("═══════════════════════════════════════════════════════════════════════════");
//
//            for (org.report.PosTobase.getTableID.BaseIdInfo baseInfo : baseIdsFromService) {
//              log.info("│ Base Name: {}", baseInfo.getBaseName());
//              log.info("│ Base ID:   {}", baseInfo.getBaseId());
//              log.info("│ Path:      {}", baseInfo.getPath());
//
...
//              log.info("│ Tables ({})", tableIds.size());
//
//              if (tableIds.isEmpty()) {
//                log.info("│   (không có tables)");
//              } else {
//                for (org.report.PosTobase.getTableID.TableIdInfo tableInfo : tableIds) {
//                  log.info("│   - Table ID: {} | Name: {}", tableInfo.getTableId(), tableInfo.getTableName());
//                }
//              }
//              log.info("├───────────────────────────────────────────────────────────────────────────");
//            }
...
//                  }
//                } else {
//                  // Nếu không có table, vẫn thêm base vào mapping
//                  mappings.add(new org.report.PosTobase.dto.BaseTableMapping(
//                          baseName,
//                          baseInfo.getBaseId(),
//                          null,
...
//              log.info("✅ Đã lưu {} Base-Table mappings vào service", mappings.size());
//            }
//          } catch (Exception e) {
//            log.warn("⚠️ Không thể lấy Base IDs và Table IDs từ getTableID service: {}", e.getMessage());
//          }
//        }
//
//        // Trigger refresh report với token mới (sẽ tự động lấy table IDs)
//        if (reportService != null) {
//          reportService.refreshReport();
//        }
//      } catch (Exception e) {
//        // Lưu lỗi vào session để hiển thị ở trang token
...
//    }
//
//    // Redirect đến URL ngắn gọn
//    return "redirect:/token";
//  }
//
//  // Endpoint đơn giản để xem token (URL ngắn gọn)
//  @GetMapping("/token")
...
//
//    // Build auth URL để hiển thị nút login (cần khi có lỗi hoặc chưa login)
//    String baseUrl = "https://open.larksuite.com/open-apis/authen/v1/index";
//    String authUrl = baseUrl
//        + "?app_id=" + URLEncoder.encode(appId, StandardCharsets.UTF_8)
//        + "&redirect_uri=" + URLEncoder.encode(redirectUri, StandardCharsets.UTF_8)
...
//    model.addAttribute("authUrl", authUrl);
//
//    // Kiểm tra error từ session hoặc flash attribute (từ redirect)
...
//      // Xóa khỏi session sau khi đã đọc
...
//    }
//
//    // Thêm thông tin Base ID và Table ID mặc định dùng để tạo bản ghi
...
//
//  /**
//   * Lấy app token từ wiki node API
...
//    }
//    return "token"; // có thể tạo view riêng nếu cần
//  }
//
//  /**
//   * Lấy danh sách tables từ bitable (filter tên chứa "_")
...
//
//  /**
//   * Endpoint tích hợp: Lấy app token từ wiki rồi lấy danh sách tables
...
//      Model model) {
//    try {
//      // Bước 1: Lấy app token từ wiki
//      String appToken = authService.getAppTokenFromWiki(userAccessToken, wikiToken);
//      model.addAttribute("appToken", appToken);
//
//      // Bước 2: Lấy danh sách tables (filter tên chứa "_")
//      List<larkBitableTablesResponse.TableInfo> tables =
//          authService.getTablesFilteredByName(userAccessToken, appToken);
//      model.addAttribute("tables", tables);
//      model.addAttribute("count", tables.size());
...
//
//          // Cập nhật vào reportProps nếu có
//          if (reportProps != null) {
//            reportProps.setUserAccessToken(newToken);
//          }
//        }
//        redirectAttributes.addFlashAttribute("success", "Token đã được làm mới thành công!");
//      } else {
//        redirectAttributes.addFlashAttribute("error", "Không thể làm mới token. Vui lòng đăng nhập lại.");
//      }
//    } else {
//      redirectAttributes.addFlashAttribute("error", "TokenRefreshScheduler không khả dụng");
//    }
//
//    return "redirect:/token";
//  }
